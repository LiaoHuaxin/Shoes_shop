<?php
	...
	if(isset($_POST["action"])&&($_POST["action"]=="delete")){	
		$sql_query = "DELETE FROM `shopping_cart` WHERE `cID`=?";
		$stmt = $db_link -> prepare($sql_query);
		$stmt -> bind_param("i", $_SESSION["login_person_ID"]);
		$stmt -> execute();
		$stmt -> close();
		$db_link -> close();
		//重新導向回到主畫面
	...
	FROM `goods`, `shopping_cart` WHERE `goods`.`cNumber`=`shopping_cart`.`cNumber` AND `shopping_cart`.`cID`=?";
			...
			<div class = "login">
				<div class = "login_zone" style = "margin-top: 120px; width: 450px; height: 450px;">
					<div class = "login_member">
						<h4 style="font-size: 0.7em;">是否要刪除所有產品</h4>
					</div>
					<div class = "login_member">
					</div>
					<input name="action" type="hidden" value="delete">
					<input type="submit" name="button" id="button" value="刪除所有產品">
